...
def compute_fitness(ret,risk)


class Particle:
  def __init__(self,_ndim, nhood_size):
    self.fit = self.fitnbest = self.fitbest = 0
...
def pso_solve(self, popsize, idof, numvar, niter, nhood_size):
  rnd.seed(550)
  ...
  for i in range(popsize):
    ...
    pop[i].fit = compute_fitness(idof,pop[i].x)
